...
function applyChanges(doc, incoming) {
  ...
  /* A "change" that matches what is already there is not a change. Dropping it
     ...
     Both sides go through the sanitiser first, and that is not decoration.
     `new URL(x).href` NORMALISES — a bare origin comes back with a trailing
     slash, an uppercase host comes back lowercased — so comparing a sanitised
     submission against the raw text of the file reports every URL on the page
     as edited the first time anybody presses Save. Same code path on both
     sides means the two cannot drift apart later either.

     A field whose stored value the sanitiser REJECTS falls back to comparing
     the raw text, so a link the rules would not accept today is left exactly
     as it is rather than being quietly rewritten by an unrelated save. */
  for (const key of Object.keys(applied)) {
    const current = copy.sanitise(key, doc.values[key]) ?? doc.values[key];
    if (applied[key] === current) delete applied[key];
  }

  const files = [];
  if (!Object.keys(applied).length) return { files, applied };

  // A key can live in more than one file — a social destination is in three —
  // so the set is the union of every file every changed key belongs to.
  const touched = new Set(
    Object.keys(applied).flatMap((key) => copy.filesOf(copy.BY_KEY.get(key)))
  );
 ...
 */
/** What the key prefixes are called in a sentence. `social` is not a page. */
const AREAS = {
  home: 'the home page',
  shop: 'the shop',
  product: 'the product page',
  social: 'the social buttons',
};

const areaOf = (keys) =>
  [...new Set(keys.map((k) => AREAS[k.split('.')[0]] ?? k.split('.')[0]))].sort().join(' and ');

const count = (n, one, many) => `${n} ${n === 1 ? one : many}`;

function commitMessage(applied) {
  const keys = Object.keys(applied).sort();
  const links = keys.filter((k) => copy.BY_KEY.get(k)?.kind === 'url');
  const words = keys.filter((k) => !links.includes(k));

  /* Three branches rather than one, because "Reword 1 string on the social
     page" was all three of wrong: a destination is not wording, there is no
     social page, and the one thing a copy commit is read for a year later is
     "what moved". */
  let subject;
  if (!words.length) subject = `Repoint ${count(links.length, 'social link', 'social links')}`;
  else if (!links.length) subject = `Reword ${count(words.length, 'string', 'strings')} on ${areaOf(words)}`;
  else subject =
    `Reword ${count(words.length, 'string', 'strings')} on ${areaOf(words)}, ` +
    `and repoint ${count(links.length, 'link', 'links')}`;

  return [subject, '', ...keys.map((key) => `- ${key}`), '', 'Edited at /admin/edit.'].join('\n');
}
