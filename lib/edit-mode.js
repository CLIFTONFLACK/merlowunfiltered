...
/* :invalid rather than a check of our own — the browser already knows what a URL
   is, and type="url" makes it say so. Only ever seen on a url field, because a
   text input has nothing to be invalid about. */
.edit-field__input:invalid { border-color: var(--red-hot); }
.edit-field__input:invalid:focus { outline-color: var(--red-hot); }
...
/* type="url" is not cosmetic: it puts a URL keyboard on a phone, and it makes
   the browser mark a malformed value :invalid so you see it while typing rather
   than on save. The server refuses anything but http and https regardless —
   this is the fast feedback, not the gate. spellcheck off because a red squiggle
   under every URL trains you to ignore red squiggles. */
const field = (entry, value) => {
  const isUrl = entry.kind === 'url';
  return `
      <label class="edit-field">
        <span class="edit-field__label">${esc(entry.label ?? entry.key)}</span>
        <input class="edit-field__input" maxlength="${entry.max}"
               type="${isUrl ? 'url' : 'text'}"${isUrl ? ' inputmode="url" spellcheck="false" placeholder="https://"' : ' spellcheck="true"'}
               data-key="${esc(entry.key)}" value="${esc(value ?? '')}">
      </label>`;
};
...
(function () {
  ...
  /* ── Text marks and attribute marks ────────────────────────────────────
     Most marks sit on an element whose TEXT is the string. A few sit on an
     element one of whose ATTRIBUTES is — a <meta content>, or a social button
     whose href is the editable part and whose only content is an icon. Those
     carry data-edit-attr, and they must never be armed or read as text: an
     anchor wrapping an SVG has no text, so collecting it would read every one
     of those keys as empty and the page would open claiming four unsaved
     changes nobody made. They are edited from the panel instead. */
  function isText(el) {
    return !el.hasAttribute('data-edit-attr');
  }
  ...
  function collect() {
    ...
    for (var i = 0; i < nodes.length; i++) {
      if (chrome.contains(nodes[i]) || !isText(nodes[i])) continue;
  ...
  function mirror(key, value, from) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      if (nodes[i] === from || chrome.contains(nodes[i]) || !isText(nodes[i])) continue;
  ...
  function arm(root) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      var el = nodes[i];
      if (chrome.contains(el) || el.isContentEditable || !isText(el)) continue;
