...
/**
 * Whose token is this, and may it write here?
 *
 * Worth asking separately, because the obvious check does not work: this
 * repository is PUBLIC, so reading a ref succeeds for any valid token at all —
 * including one belonging to an entirely different account. Every read passing
 * therefore says nothing about whether a save will, and the first sign of
 * trouble would otherwise be a 403 landing on somebody who has just typed a
 * paragraph.
 *
 * `permissions.push` on the repository is the answer GitHub gives to exactly
 * this question, for classic and fine-grained tokens alike. `login` is worth
 * carrying because the likeliest cause of a "no" is a token minted while
 * signed in as the wrong one of several accounts, and that is invisible from
 * anywhere else.
 *
 * @returns {Promise<{login: string|null, canWrite: boolean, error: string|null}>}
 *   Never throws. Not being able to ask is itself an answer worth showing.
 */
async function writeAccess(env = process.env) {
  const out = { login: null, canWrite: false, error: null };
  if (!configured(env)) {
    out.error = 'No token is set.';
    return out;
  }

  try {
    out.login = (await call('/user', { env }))?.login ?? null;
  } catch (err) {
    // A fine-grained token can be denied /user while still working on a repo,
    // so this is not fatal — it only costs the name in the message.
    out.login = null;
  }

  try {
    const info = await call(`/repos/${repo(env)}`, { env });
    out.canWrite = info?.permissions?.push === true;
  } catch (err) {
