 ...
 * ── Why the <base> ───────────────────────────────────────────────────────
 *
 * The page is served from /admin/edit rather than from where it lives, and
 * index.html asks for its assets by RELATIVE path — `css/styles.css`,
 * `media/merlow-wordmark.png`, `js/main.js`. Those resolve against the
 * current URL, so at /admin/edit they become /admin/css/styles.css and every
 * one of them 404s: no stylesheet, no wordmark, no video. The page still
 * "works" in the sense that every string is there and editable, which is
 * exactly why this got shipped — the DOM was checked and the page was not
 * looked at.
 *
 * shop.html and product.html happen to use absolute paths and so never showed
 * it. Making index.html match them was the other option and was rejected:
 * that file is opened straight off disk during development (see the README),
 * and absolute paths break a file:// open. One line here fixes every page,
 * including any added later, and changes nothing about what the site serves.
 *
 * It goes immediately after <head> because a <base> only governs what follows
 * it, and the stylesheet link is the first thing that needs it.
 */
