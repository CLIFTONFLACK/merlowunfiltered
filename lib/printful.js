...
async function shape({ sync_product, sync_variants }) {
  ...
  const variants = synced.map((v) => {
    ...
    return {
      ...
      name: v.name,
      // What distinguishes this variant from its siblings, and nothing else.
      label: variantLabel(v.name, sync_product.name, cv),
  ...
  /* A product that comes one way has nothing to disambiguate. Left alone, the
     cap would be sold as "Adidas Alliance — Black — Black / White / One size",
     which is accurate and says nothing. */
...
/* Printful names a variant by repeating the whole product name and appending
   the options: "Denim T-Shirt (Official MERLOW - UNFILTERED) / Black / S". Used
   as-is next to the product name it produces the name twice in one line — which
   is what the Stripe checkout page and the emailed receipt were showing.

   The catalog record already has the options as fields, so they are preferred.
   Where there is no catalog record the product name is cut off the front of the
   variant name instead, which is the same answer by a worse route. */
function variantLabel(variantName, productName, catalogVariant = {}) {
  const options = [catalogVariant.color, catalogVariant.size].filter(Boolean);
  if (options.length) return options.join(' / ');

  const name = String(variantName || '').replace(/\s+/g, ' ').trim();
  const product = String(productName || '').replace(/\s+/g, ' ').trim();
  const trimmed = product && name.startsWith(product) ? name.slice(product.length) : name;
