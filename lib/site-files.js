...
/* Reading the site's own files from inside a function.
   ...
   1. Vercel bundles a function with the files it can SEE it uses, by reading
      ...
      The `functions.includeFiles` entry in vercel.json is the ONLY thing that
      puts them in the bundle. This was once written the other way round — that
      the LITERAL table below, being `path.join(__dirname, '..', 'a-string')`,
      would be followed by Vercel's file tracer and so served as a second
      guard. It is not, and index.html shipped missing from includeFiles on the
      strength of that belief: it worked on a laptop and /admin said "index.html
      could not be read" in production.

      So: a new editable file must be added to vercel.json as well as to the
      table below, and the test named "vercel.json bundles every file the schema
      names" is what enforces it. A comment claiming something is guarded is not
      a guard.
